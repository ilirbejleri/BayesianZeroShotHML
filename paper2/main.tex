...
We test this on six image classification datasets spanning attribute-based and text-based class embeddings, and from very flat (Stanford Cars) to deeply hierarchical (iNaturalist) label spaces.
We find that the relationship is \emph{not} universal.
Across the six datasets, the per-dataset slope of $c$ vs.\ $\log N$ varies from $+1.31$ ($R^2=0.95$, AwA2) all the way to $-0.17$ ($R^2=0.86$, SUN397) --- a directionally inconsistent range that decisively rules out a single scaling law.
...
We trained the same model architecture --- with no hyperparameter changes --- on six image classification datasets covering a wide range of taxonomic structures and class-embedding modalities: AwA2~\cite{xian2018awa2}, CIFAR-100~\cite{krizhevsky2009cifar}, CUB-200~\cite{wah2011cub}, SUN397~\cite{xiao2010sun}, Stanford Cars~\cite{krause2013cars}, and iNaturalist~\cite{vanhorn2021inaturalist}.
Each was run through the same data-ablation sweep at $\{1, 5, 10, 25, 50, 100\}\%$ of the training set.
A seventh dataset (tiered-ImageNet~\cite{ren2018tiered}) is in progress and excluded from the main analysis; partial results are reported in the appendix.

\paragraph{What we found.}
The $c \propto \log N$ pattern does \emph{not} generalise.
Fitting $c = a \log N + b$ separately per dataset, the slope $a$ varies from $+1.31$ (AwA2) to $-0.17$ (SUN397), changing sign across the six datasets.
...
AwA2 is the only dataset using expert-annotated per-class binary attributes (85-d, e.g., \texttt{has\_stripes}, \texttt{carnivorous}); the other five use 300-d GloVe word vectors~\cite{pennington2014glove} derived from class names.
...
\begin{enumerate}
    \item We refute the claim that learnable curvature in hyperbolic neural networks obeys a universal $c \propto \log N$ scaling law, using six datasets in a controlled experimental setup that isolates dataset and ablation as the only varying factors.
...
\Cref{tab:datasets} lists the six datasets and their key properties.
The two columns most relevant for our analysis are the \textbf{embedding modality} and the \textbf{tree depth}.
Among our six datasets, only AwA2 uses expert-annotated per-class binary attributes; the other five use GloVe word vectors derived from class names (with WordNet lookup for tiered-ImageNet's wnids and a taxonomy-name parse for iNaturalist's directory-encoded names).

\begin{table}[t]
\centering
\caption{The six datasets used in this study, with class-embedding modality and ground-truth taxonomy metrics. Hierarchy metrics are computed from the simplified taxonomies in our \texttt{hierarchy.py}; for tiered-ImageNet and iNaturalist these are conservative truncations of the full WordNet / biological hierarchies.}
\label{tab:datasets}
...
We deliberately span a range of label-space structures: shallow non-taxonomic domains (Stanford Cars region/make groupings, SUN397 scene categories), exact taxonomies (CIFAR-100's 20-superclass partitioning), biological/ornithological hierarchies (CUB-200, iNaturalist), and the AwA2 attribute-based animal description.
...
\begin{table}[t]
\centering
\caption{Converged curvature $c$ at epoch 200 across all six completed datasets and data-ablation fractions. AwA2's monotonic increase is the dramatic outlier; the GloVe-embedded datasets show much more modest curvature, with three of them ending at or near (and one well below) their initial $c_0 = 1.0$.}
...
\caption{Converged curvature vs.\ training data fraction (log-x) for all six datasets. AwA2 (top) climbs almost linearly in $\log N$. The GloVe-embedded datasets cluster between $c = 0.8$ and $c = 2.3$ and exhibit qualitatively different trajectories: CUB-200 and Stanford Cars rise then plateau; SUN397 and iNaturalist peak early and decline.}
...
The signs and magnitudes of $a$ disagree wildly: the AwA2 slope is roughly an order of magnitude larger than CUB-200's (the next-largest positive), and three datasets have \emph{negative} slopes, one of which (SUN397, $a = -0.17$) fits very tightly ($R^2 = 0.86$) to a clean monotonic decline.
...
\caption{Per-dataset $c = a\log N + b$ fits. AwA2's high-$R^2$, high-slope fit is what motivated the original universal-scaling reading. The other five datasets fit either poorly (CIFAR-100, iNaturalist) or to slopes near zero or negative, contradicting that reading.}
...
SUN397, iNaturalist, and (in our partial results) tiered-ImageNet.
...
SUN397 (depth 3) and iNaturalist (depth 4), the two datasets with the deepest taxonomies among the GloVe-embedded ones, are also the two with the lowest converged curvature.
A simple linear regression of $c_{100\%}$ on tree depth across the six datasets has $R^2 < 0.05$ once AwA2 is excluded, and is essentially driven by AwA2 alone when it is included.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/curvature_vs_hierarchy.pdf}
\caption{Converged curvature at 100\% data vs.\ (left) taxonomy tree depth and (right) mean branching factor. The strong-looking correlations are driven entirely by AwA2 as a single high-curvature outlier; among the five GloVe-embedded datasets, neither metric predicts curvature well.}
...
By contrast, the other five datasets use 300-d GloVe word vectors derived from class names (or, for tiered-ImageNet and iNaturalist, from taxonomy-derived words).
...
A secondary observation: the three datasets in Pattern C (declining curvature) are also our three largest by class count: SUN397 (397), iNaturalist (500), and tiered-ImageNet (608, partial).
...
Their reported $c \approx 0.113$ on tiered-ImageNet 1-shot is in the same general range as our partial tiered-ImageNet results (final curvatures between 1.0 and 1.8 across ablations, declining from earlier peaks).
Our setup uses a different training objective (ZSL geodesic alignment) and a different evaluation protocol, so a precise numerical comparison is not meaningful.
What is meaningful is that both papers find tiered-ImageNet's optimal/converged curvature in roughly the same low-to-moderate range, consistent with the more general observation that text-derived class embeddings on large class sets produce only moderate curvature.
...
Six datasets is a thin slice of the space.
...
It does not.
The within-dataset slope of $c$ vs.\ $\log N$ varies from $+1.31$ (AwA2) to $-0.17$ (SUN397), and the trajectories cluster into three distinct response patterns.
